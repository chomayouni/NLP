...
def topic_modeling(bag_of_words_matrix, vectorizer, num_topics, specific_words):
    words = list(vectorizer.vocabulary_.keys())
    id2word = Dictionary([words])
    corpus = [id2word.doc2bow(text) for text in bag_of_words_matrix]
    ...
    word_indices = [id2word.token2id.get(word, 0) for word in specific_words]
    specific_word_dist = topic_word_dist[:, word_indices]
    lda_mat = np.array(specific_word_dist, dtype='float64')

    return lda_mat

def build_lda(hero_quotes, villain_quotes):
    # Combine hero and villain quotes into one list of documents
    documents = [word_tokenize(quote) for quote in hero_quotes + villain_quotes]
    ...
    # Build the LDA model with 2 topics (hero and villain)
    lda_model = LdaModel(corpus=corpus, num_topics=2, id2word=id2word)
...
def main():
    ...
    if not args.standard and not args.custom:
        ...
        num_topics = 2
        specific_words = ['word1', 'word2', 'word3', 'word4', 'word5', 'word6', 'word7', 'word8', 'word9', 'word10']

        # Perform topic modeling
        lda_mat = topic_modeling(bag_of_words_matrix, vectorizer, num_topics, specific_words)

        # Print the matrix
        print(lda_mat)

        lda_model = build_lda(hero_quotes, villain_quotes)
